...
class PolicyGradientREINFORCE_user(object):
  ...
  def create_variables(self):
    ...
    with tf.name_scope("model_inputs_user"):
    ...
    with tf.name_scope("predict_actions_user"):
      # initialize policy network
      with tf.variable_scope("policy_network_user"):
    ...
    with tf.name_scope("compute_pg_gradients_user"):
      # gradients for selecting action from policy network
      self.taken_actions = tf.placeholder(tf.int32, (None,), name="taken_actions")
      self.discounted_rewards = tf.placeholder(tf.float32, (None,), name="discounted_rewards")

      with tf.variable_scope("policy_network_user", reuse=True):
      ...
      # emit summaries
      tf.summary.scalar("policy_loss_user", self.pg_loss)
      tf.summary.scalar("reg_loss_user", self.reg_loss)
      tf.summary.scalar("total_loss_user", self.loss)

    # training update
    with tf.name_scope("train_policy_network_user"):
      # apply gradients to update policy network
      self.train_op = self.optimizer.apply_gradients(self.gradients)

    with tf.name_scope("SL_parts_user"):
  ...
  def storeRollout(self, state, action, reward, rnn_states, bit_vecs, user_utt, real_action=None):
