...
class SysCommonNlg(object):
    ...
    """
    templates = {
        SystemAct.ASK_TYPE: ["Do you have a preference on the [informable_slot]?"],
    ...
    # UserAct.GOODBYE: ["Thank you for helping me! Goodbye"],
        ...
        UserAct.INFORM_TYPE: [{"food": "I am looking for a restaurant that serves [food] food.",
                              ...
                              {"food": "i would like the [food] food.",
                               ...
                               "area_pricerange": "is there a [pricerange] place to eat in the [area] ?",
...
#
    ...
    def generate_sent(self, actions, templates=UsrCommonNlg.templates,
                      generator=None, context=None, turn_num=0,
                      seq2seq=None):
        """
        ...
        if generator is None and seq2seq is None:
                ...
                #         sent = sent.replace("[time]", a.parameters['time'])
                    ...
                    if a_copy2.act != UserAct.ASK_INFO:
                        ...
                        if len(dontcare_type) == 1:
                            sent += ' i don\'t care about the ' + dontcare_type[0] + '.'
                        elif len(dontcare_type) == 2:
                            sent += ' i don\'t care about the ' + dontcare_type[0]
                            ' or ' + dontcare_type[1] + '.'
                        elif len(dontcare_type) == 3:
                            sent += ' i don\'t care about the ' + dontcare_type[0] + ',' + dontcare_type[1] + ' or ' + \
                                    dontcare_type[2] + '.'
        ...
        elif generator is not None and (seq2seq is None):
            actions_without_parameters = [UserAct.ANYTHING_ELSE, UserAct.GOODBYE]
            actions_with_parameters = [UserAct.INFORM_TYPE,
                                       UserAct.INFORM_TYPE_CHANGE,
                                       UserAct.ASK_INFO,
            ...
            for a in actions:
                a_copy = copy.deepcopy(a)
                if a.act in actions_without_parameters:
                    sent = generator.retrieve(context=context, act_param=a.act)
                    str_actions.append(sent)
                    # str_actions.append(self.sample(templates[a.act][('',)]))
                    ...
                    if len(sorted_ps) > 0:
                        act_param = a.act + "-" + "-".join(sorted_ps)
                    else:
                        act_param = a.act
                    sent = generator.retrieve(context=context, act_param=act_param)
                    for p, v in a.parameters.items():
                        if p != 'name':
                            sent = sent.replace("value_"+p, v)
                        else:
                            sent = sent.replace("restaurant_name", v)
                    str_actions.append(sent)
                    ...
                    sorted_ps = tuple(sorted([p for p in a.parameters.keys()]))#[p for p in a.parameters]
                    if len(sorted_ps) > 0:
                        act_param = a.act + "-" + "-".join(sorted_ps)
                    else:
                        act_param = a.act
                    ...
                    # sent = self.sample(templates[a.act][args]) # "what is the following information? " + " ".join(args) + "?"
                    ...
                    sorted_ps = tuple(sorted(["value_"+p for p in a.parameters]))
                    if len(sorted_ps) > 0:
                        act_param = a.act + "-" + "-".join(sorted_ps)
                    else:
                        act_param = a.act
                    ...
                    for p, v in a.parameters.items():
                        # print("v: ", v)
                        if p == 'people':
                            sent = sent.replace("value_people", v)
                        else:
                            sent = sent.replace("value_"+p, v)
                    str_actions.append(sent)
            ...
            for a in actions:
                ...
                # pdb.set_trace()
                if a.act in actions_without_parameters:
                    tmp_source = " ".join(a.act.lower().split('_'))
                    target = " ".join(seq2seq(tmp_source))
                    target = target.replace("<eos>", "")
                    str_actions.append(target)

                else:
                    tmp_args = {}
                    for k, v in a.parameters.items():
                        if k == 'people':
                            tmp_args["value_people"] = v
                        elif k == 'name':
                            tmp_args["restaurant_name"] = v
                        elif k in ['address', 'postcode', 'phone']:
                            tmp_args[k] = v
                        else:
                            tmp_args["value_"+k] = v
                    tmp_source = " ".join(a.act.lower().split('_') + sorted(list(tmp_args.keys())))
                    # pdb.set_trace()
                    target = " ".join(seq2seq(tmp_source))#.replace("<eos>", "")
                    target = target.replace("<eos>", "")
                    for p, v in tmp_args.items():
                        if p not in target:
                            if p == 'value_time':
                                target = target + ' at ' + v
                            elif p == 'value_day':
                                ...
                                target = target + ' for ' + v + ' people'
                            elif p == 'value_food':
                                target = target + ' ' +  v + ' food'
                            elif p == 'value_area':
                                target = target + ' ' + v + ' area'
                            elif p == 'value_pricerange':
                                target = target + ' ' + v + ' pricerange'


                        else:
                            if v is not None:
                                target = target.replace(p, v)
                            else:
                                pass
                    # pdb.set_trace()
                    target = target.replace("<eos>", "")
