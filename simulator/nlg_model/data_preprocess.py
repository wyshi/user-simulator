...
def tokenizer(text):  # create a tokenizer function
    # print(text)
    text = [tok for tok in word_tokenize(text) if tok != ' ' and tok != '[' and tok != ']']
    # print(text)
    tokenized_text = []
...
class DataProcessor(object):
    ...
    def process_data(self):
        ...
        # Define field
        SOURCE = data.Field(sequential=True, tokenize=tokenizer, use_vocab=True, lower=True, eos_token='<eos>',
                          batch_first=True, truncate_first=True, include_lengths=True)
        TARGET = data.Field(sequential=True, tokenize=tokenizer, use_vocab=True, lower=True, fix_length=config.max_utt_len,
                            init_token='<go>',
                            eos_token='<eos>',
                          batch_first=True, truncate_first=True, include_lengths=True)
        ...
        for col in df.columns.tolist():
            if col == config.source_name:
                datafields.append((col, SOURCE))

            elif col == config.target_name:
                datafields.append((col, TARGET))
            #         elif col == "Index":
...
#
        ...
        TARGET.build_vocab(train.source, train.target, valid.source, valid.target,
                           test.source, test.target, vectors=vectors, max_size=30000)
        ...
        print('num of tokens in source', len(SOURCE.vocab.itos))
        print('num of tokens in target', len(TARGET.vocab.itos))

        print('most common in target', TARGET.vocab.freqs.most_common(5))

        print('len(train)', len(train))
        print('len(test)', len(test))

        self.train_iter = data.Iterator(dataset=train, batch_size=config.batch_size, train=True, sort_key=lambda x: len(x.source),
                                   sort_within_batch=False, repeat=False, shuffle=True,
                                   device='cuda:0' if config.use_gpu else -1)

        self.valid_iter = data.Iterator(dataset=valid, batch_size=config.batch_size, train=False, sort_key=lambda x: len(x.source),
                                 sort_within_batch=False, repeat=False,
                                 device='cuda:0' if config.use_gpu else -1)

        self.test_iter = data.Iterator(dataset=test, batch_size=config.batch_size, train=False, sort_key=lambda x: len(x.source),
                                  sort_within_batch=False, repeat=False,
                                  device='cuda:0' if config.use_gpu else -1)

        self.num_tokens = len(TARGET.vocab.itos)
    ...
    def batchlize_data(self):
        ...
        self.train_iter = BatchWrapper(self.train_iter, config.source_name, config.target_name)
        self.valid_iter = BatchWrapper(self.valid_iter, config.source_name, config.target_name)
        self.test_iter = BatchWrapper(self.test_iter, config.source_name, config.target_name)

    def process_one_data(self, text):
        # Input data
        text = self.SOURCE.preprocess(text)
        text = [[self.SOURCE.vocab.stoi[x] for x in text]]
