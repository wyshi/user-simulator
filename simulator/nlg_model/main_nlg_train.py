...
from simulator.nlg_model.model import NLG_model
from simulator.nlg_model.data_preprocess import DataProcessor, l2_matrix_norm, AverageMeter, print_cm, eval_
...
def load_nlg_model(model_dir="simulator/nlg_model/model/model-test-30-new.pkl"):
    model = torch.load(model_dir, map_location='cpu')
    data_processor = DataProcessor()
    net = NLG_model(data_processor=data_processor)
    net.load_state_dict(model)
    net.eval()
    return net

def single_pred(net, sent, n_sent=1):

    input_seqs = net.data_processor.process_one_data(sent)
    # input_seqs = Variable(input_seqs)
    input_seq_lens = torch.LongTensor([input_seqs.shape[1]])
    # decoded = net(input_seqs, input_seq_lens, is_decode=True)
    go_token_id = net.data_processor.TARGET.vocab.stoi['<go>']
    target_seqs = torch.tensor([[go_token_id]*n_sent], dtype=torch.int64)
    decoded = net(input_seqs, input_seq_lens, target_seqs,
        target_lens=None, hidden=None, is_decode=True)

    decoded_sent = []
    for i in decoded:
        if i.item() == net.data_processor.TARGET.vocab.stoi['<eos>']:
            break
        token = net.data_processor.TARGET.vocab.itos[i.item()]
...
if __name__ == '__main__':
    ...
    data_processor = DataProcessor()
    net = NLG_model(data_processor=data_processor)
    net.optimizer = optim.Adam(params=net.parameters(), lr=1e-3, weight_decay=1e-2)
    ...
    # # decoded = net(input_seqs, input_seq_lens, is_decode=True)
    # go_token_id = net.data_processor.TARGET.vocab.stoi['<go>']
    # target_seqs = torch.tensor([[go_token_id]*1], dtype=torch.int64)
    # decoded = net(input_seqs, input_seq_lens, target_seqs,
    #     target_lens=None, hidden=None, is_decode=True)
    ...
    for epoch in range(epochs):
        ...
        for phase in ('train', 'val'):
            ...
            for l in tqdm(phrase_iter):
                ...
                (input_seqs, input_seq_lens), (target_seqs, target_seq_lens) = l

                net.optimizer.zero_grad()  # clear the gradient

                logits = net(input_seqs, input_seq_lens, target_seqs,
                             target_lens=None, hidden=None, is_decode=IS_DECODE)
                loss = net.supervised_loss(logits, target_seqs)
                acc = 0#eval_(logits, labels=ys.data.long())
                if phase == 'train':
                    loss.backward()
            ...
            # import pdb
            # pdb.set_trace()
            if phase == 'val' and epoch_loss[phase][-1] < best_loss:
                ...
                for l in tqdm(net.test_iter):
                    ...
                    (input_seqs, input_seq_lens), (target_seqs, target_seq_lens) = l
                    net.optimizer.zero_grad()  # clear the gradient
                    sents = net.test(input_seqs, input_seq_lens)

                    original_source = []
                    for seq in input_seqs:
                        s = " ".join([net.data_processor.TARGET.vocab.itos[i.item()] for i in seq])
                        original_source.append(s)

                    original_target = []
                    for seq in target_seqs:
                        s = " ".join([net.data_processor.TARGET.vocab.itos[i.item()] for i in seq])
                        original_target.append(s)

                    print(list(zip(original_source, original_target, sents))[:5])
                    # print(sents[:5])
                #     output = logits
    ...
    print('[Info] best valid acc: {:.2%} at {}th epoch'.format(best_loss, best_epoch))
